...
DRIVER_UNLOAD TinyEdrUnload;
DRIVER_DISPATCH TinyEdrCreateClose, TinyEdrRead;
...
Globals g_Globals; //initialize the global object
...
extern "C" NTSTATUS DriverEntry(PDRIVER_OBJECT DriverObject, PUNICODE_STRING) {
	...
	UNICODE_STRING symLink = RTL_CONSTANT_STRING(L"\\??\\tinyedr");
	bool symLinkCreated = false;
	bool processCallbacks = false, threadCallbacks = false, loadImageCallback = false;

	do {
		UNICODE_STRING devName = RTL_CONSTANT_STRING(L"\\Device\\tinyedr");
		status = IoCreateDevice(DriverObject, 0, &devName, FILE_DEVICE_UNKNOWN, 0, TRUE, &DeviceObject);
		if (!NT_SUCCESS(status)) {
			KdPrint((DRIVER_PREFIX "failed to create device (0x%08X)\n", status));
			break;
		}
		DeviceObject->Flags |= DO_DIRECT_IO; //do direct I/O
	...
	DriverObject->DriverUnload = TinyEdrUnload;
	DriverObject->MajorFunction[IRP_MJ_CREATE] = DriverObject->MajorFunction[IRP_MJ_CLOSE] = TinyEdrCreateClose;
	DriverObject->MajorFunction[IRP_MJ_READ] = TinyEdrRead;

	return status;
}

//for completing a request
NTSTATUS TinyEdrCreateClose(PDEVICE_OBJECT, PIRP Irp) {
...
//unload routine
void TinyEdrUnload(PDRIVER_OBJECT DriverObject) {
	//unload routines for callbacks here...
...
NTSTATUS TinyEdrRead(PDEVICE_OBJECT, PIRP Irp) {
...
void OnProcessNotify(PEPROCESS Process, HANDLE ProcessId, PPS_CREATE_NOTIFY_INFO CreateInfo) {
	...
	if (CreateInfo) {
		...
		if (info == nullptr) {
			KdPrint((DRIVER_PREFIX "faled allocation\n"));
			return;
		}
